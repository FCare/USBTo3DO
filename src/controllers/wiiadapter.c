...
bool map_wii_classic_adapter(uint8_t instance, uint8_t *id, controler_type *type, void **res, void *ctrl_v)
 {
  ...
  *id = ctrl->index + instance;
  ...
  result->X = btn->TRIGGER || btn->BASE4;
  result->P = btn->BASE3;
  result->A = btn->TOP;
  result->B = btn->THUMB2;
  result->C = btn->THUMB;
  result->L = btn->BASE || btn->TOP2 || (btn->ABS_RY >= 192);
  result->R = btn->BASE2 || btn->PINKIE || (btn->ABS_RY <= 64);
